...
# Used to ensure user redirected to login page if attempting to access when logged out
@login_required
def logout(request):
    '''
    Log the user out
...
def login(request):
    ...
    Return a login page
    '''
    # Ensures that user redirected to index if accessing login, when already logged in
